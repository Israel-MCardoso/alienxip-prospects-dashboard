...
drop policy if exists "users can update commercial tasks" on public.commercial_tasks;
...
drop policy if exists "authenticated users can read wiki" on public.wiki_pages;
...
drop policy if exists "authenticated users can create files" on public.files;
