...
export async function createProspectAction(formData: FormData) {
  ...
  const { data: prospect, error } = await supabase.from("prospects").insert({
    ...
    address_street: emptyToNull(input.address_street),
    address_number: emptyToNull(input.address_number),
    address_complement: emptyToNull(input.address_complement),
...
export async function duplicateProspectAction(id: string) {
  ...
  const { data: newProspect, error: insertError } = await supabase
    ...
    .insert({
      ...
      address_number: prospect.address_number,
      address_complement: prospect.address_complement,
