...
export function WikiList({ pages, error }: { pages: WikiPageRow[]; error: string | null }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card><CardHeader><CardTitle>Criar página</CardTitle></CardHeader><CardContent><WikiForm /></CardContent></Card>
      <Card><CardHeader><CardTitle>Páginas</CardTitle><CardDescription>{pages.length} registro(s)</CardDescription></CardHeader><CardContent className="flex flex-col gap-3">
        <form className="grid gap-2 md:grid-cols-4"><Input name="q" placeholder="Buscar wiki" /><CategorySelect /><StatusSelect /><Button type="submit" variant="outline">Filtrar</Button></form>
